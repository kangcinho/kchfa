...
class EmitenCalculateController extends Controller
{
    public function showCalculateComparasionByEmiten(Request $request){
        $groupEmiten = new GroupingEmiten;
        $emitens = json_decode($request->emitens);
        $financials = collect();
        foreach($emitens as $emitenKode){
            $financials = $financials->merge($this->getFinancialByEmiten($emitenKode));
        }        
