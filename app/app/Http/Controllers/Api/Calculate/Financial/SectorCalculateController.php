...
class SectorCalculateController extends Controller
{
    public function showCalculateComparasionBySector(Request $request){      
        $groupEmiten = new GroupingEmiten; 
        $financials = $this->getFinancialBySector($request->sector);
    ...
    private function getFinancialBySector($sectorID){
        try{
            $financials = \DB::table('emiten_datas')
                ...
                ->where('sectors.sectorID', $sectorID)
                ->orderBy('emitenKode')
                ->get();
            return $financials;
        }catch (\Throwable $e){
