...
class SubSectorCalculateController extends Controller
{
    public function showCalculateComparasionBySubSector(Request $request){
        $groupEmiten = new GroupingEmiten;
        $subSectors = json_decode($request->subSector);
        $financials = collect();
        foreach($subSectors as $subSector){
            $financials = $financials->merge($this->getFinancialBySubSector($subSector));
        }        
    ...
    private function getFinancialBySubSector($subSectorID){
        try{
            $financials = \DB::table('emiten_datas')
                ...
                ->where('sub_sectors.subSectorID', $subSectorID)
                ->orderBy('emitenKode')
                ->get();
            return $financials;
        }catch (\Throwable $e){
